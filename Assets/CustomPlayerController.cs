...
public class CustomPlayerController : MonoBehaviour
{
	...
	/// <summary>
	/// If true, sets player movement options to flying camera.
	/// </summary>
	public bool FlyingCam = false;
	...
	protected virtual void UpdateController()
	{
        ...
        if (MoveThrottle.y > 0.0f || FlyingCam)
        {
	...
	private void FlyingCamEnable()
    {
		FlyingCam = true;
		GravityModifier = 0;
    }

	private void FlyingCamDisable()
    {
		FlyingCam = false;
    ...
    public virtual void UpdateMovement()
	{
		...
		bool pause = GameObject.Find("ReplayObjects").GetComponent<MotionReplayer>().pause;

		if (OVRInput.GetDown(OVRInput.Button.One))
        {
			Debug.Log("changing pause");
			pause = true;
        }
		...
		if (EnableLinearMovement)
		{
			...
			bool moveUp = Input.GetKey(KeyCode.Space);
            ...
            // No positional movement if we are in the air + flying cam is disabled
            if (!FlyingCam)
			...
			if (moveUp)
				MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * BackAndSideDampen * Vector3.up);
			if (moveDown)
				MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * BackAndSideDampen * Vector3.down);
